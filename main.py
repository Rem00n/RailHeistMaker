...
def sine_wave(y_offset, BGamplitude, BGfrequency, BGphase_shift, surface, color):
    points = []
    for x in range(surface.get_width()):
        y = y_offset + BGamplitude * math.sin(BGfrequency * (x + BGphase_shift))
        points.append((x, y))
...
def create_BGwave_surface(BGphase_shift, color, BGamplitude, BGfrequency,h):
    extra_width = WIDTH * 3  
    surface = pygame.Surface((extra_width, HEIGHT), pygame.SRCALPHA)
    sine_wave(HEIGHT * 0.35+h, BGamplitude, BGfrequency, BGphase_shift, surface, color)
    return surface

def drawBackground():
    global BGoffset_x_1, BGoffset_x_2
    screen.fill(BGC1)
    delta_time = clock.tick(240) / 1000
    BGoffset_x_1 -= BGspeed_x_1 * delta_time
    if BGoffset_x_1 <= -WIDTH:
        BGoffset_x_1 += WIDTH
    BGoffset_x_2 -= BGspeed_x_2 * delta_time
    if BGoffset_x_2 <= -WIDTH:
        BGoffset_x_2 += WIDTH
    screen.blit(BGwave_surface_1, (BGoffset_x_1, 0))
    screen.blit(BGwave_surface_2, (BGoffset_x_2, 0))
...
def drawBlock(block,x,y):
	...
	if showText:
		screen.blit(font.render(block, True, '#FFFFFF'), (OFF_X + x*DIM_BLK, OFF_Y + y*DIM_BLK))
...
def firstTimeSetup():
	...
	GAMECODEFILE = get_setting('codeFilePath')
	
	if not get_setting('baseMissionsExtracted'):
		extractBaseMissions()
		...
		for wLine in content:
			...
			match line[0]:
				...
				case 'newLevel.horse ':
					...
					horseButton.num = 0 if 'LEFT' in line[1] else 1
				case 'newLevel.horseRider ':
					#print('newLevel.horseRider ')
					horseRiderButton.num = 0 if 's13_Dodger' in line[1] else 1 if 's13_Brand' in line[1] else 2
				case 'trainLayout ':
					#print('trainLayout')
					Trains.append([])
				case 'trainLayout +':
...
def backupDatawin():
	DATAWINPATH = get_setting('datawinPath')
...
def insertDatawin():
	...
	insertMissionToGame(missionNameBox.text,selectedLevelInFile[0],insertOrReplaceButton.num==1)

	command = [UTMODCLIPATH,'replace',DATAWINPATH,'-c',f'gml_Object_o13_Game_Other_21={GAMECODEFILE}','-o',DATAWINPATH]
	x = subprocess.call(command)
	if x==0:
		print(f"inserted code file with mission: {missionNameBox.text} into game!")
		return
	print(f"error!! restore datawin from backup")



def cleanStringForWindowsFileName(s):
	r = ''
	for c in s:
		if c in ['\\','/',':','"','*','?','|','<','>']:
			r = r+'_'
...
def genLevelCodeBlock(missionName):
	...
	filepath = os.getcwd()+f'/missions/{cleanStringForWindowsFileName(missionName)}.txt'
...
def loadAndInsert():
	global Trains
	currentTrains = Trains.copy() ##save loaded level
	getMissionsNames()
	x = loadMissionIntoEditor()
	setRunningScreen(0)
	if x: ##user cancelled file selection
		Trains = currentTrains.copy() ##restore loaded level
		return 
	
	
	if quickInsertPositionButton.num==0: ##replace first
		insertOrReplaceButton = 1
		selectedLevelInFile[0] = 0
	elif quickInsertPositionButton.num==1: ##replace last
		insertOrReplaceButton = 1
		selectedLevelInFile[0] = len(LevelsInFile[0])
	elif quickInsertPositionButton.num==2:  ##after last
		insertOrReplaceButton = 1
		selectedLevelInFile[0] = len(LevelsInFile[0])
...
class InputBox:
	...
	def Typing(self, event):
		...
		self.refresh()
	def draw(self):
		pygame.draw.rect(screen, self.color, self.rect,0,UI_BUTTON_BORDER_RADIUS)
		pygame.draw.rect(screen, UI_BUTTON_BORDER_COLOR, self.rect,UI_BUTTON_BORDER_DEPTH,UI_BUTTON_BORDER_RADIUS)
...
class FlipButton:
	...
	def draw(self):
		pygame.draw.rect(screen, self.currentColor, self.rect,0,UI_BUTTON_BORDER_RADIUS)
		pygame.draw.rect(screen, UI_BUTTON_BORDER_COLOR, self.rect,UI_BUTTON_BORDER_DEPTH,UI_BUTTON_BORDER_RADIUS)
...
class FuncButton:
	def __init__(self, rect,text,func,arg=None,font_=None):
		self.rect = pygame.Rect(rect) 
		self.color = UI_BUTTON_COLOR
		self.hoverColor = UI_BUTTON_HCOLOR
		self.currentColor = self.color 
		if font_==None:
			...
			self.txt_surface = font_.render(text, True, "#FFFFFF") 
		self.func = func
		self.arg = arg
	def draw(self):
		pygame.draw.rect(screen, self.currentColor, self.rect,0,UI_BUTTON_BORDER_RADIUS)
...
BGamplitude = HEIGHT *0.06
BGfrequency = 2 * math.pi / WIDTH * 2
BGwave_surface_1 = create_BGwave_surface(BGphase_shift_1, BGC2, BGamplitude, BGfrequency,0)
BGwave_surface_2 = create_BGwave_surface(BGphase_shift_2, BGC3, BGamplitude*0.5, BGfrequency,230)
...
blocksInfo = {
	...
	'S': ['switch'],
	'F': ['wall switch','unused in base game'],
...
UI_BUTTON_BORDER_RADIUS = 3
UI_BUTTON_BORDER_DEPTH = 1
UI_BUTTON_BORDER_COLOR = '#CCCCCC'
...
SetupNotReadyText = TextBox("<-- Select UFO 50's data.win file to start making and inserting levels!",font,'#FF4444',150,135)
...
quickInsertPositionButton = FlipButton((280,400,100,40),0,3,["replace first","replace last","after last"])
exitButton = FuncButton((50,470,70,40),"Exit",sys.exit)

UI_0 = [SetupButton,logoText,exitButton]
...
InsertLevelButton = FuncButton((630,HEIGHT-60,140,30),'insert into game...',setRunningScreen,3)
menuButton = FuncButton((720,HEIGHT-95,50,30),'menu',setRunningScreen,0)

UI_1 = [nextTrainButton,prevTrainButton,preAddTrainButton,preRmvTrainButton,saveLevelButton,InsertLevelButton,LevelSettingsButton,blockSelectButton,menuButton]
...
insertOrReplaceButton = FlipButton((130,130,100,H),1,2,["Insert","Replace"]);
...
UI_3 = [insertOrReplaceButton,levelUpButton,levelDownButton,InsertButton,backToEditorButton,levelmaxButton,levelminButton]
...
while True:
	...
	for event in pygame.event.get():
		if event.type == pygame.QUIT or pygame.key.get_pressed()[pygame.K_F4]:
			sys.exit()
		...
		if RUNNING==0:
			if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
				...
				if get_setting('datawinLoaded') and get_setting('codeFileLoaded'):
					newMissionButton.clicked(MouseX,MouseY)
					LoadMissionButton.clicked(MouseX,MouseY)
					loadAndInsertButton.clicked(MouseX,MouseY)
					quickInsertPositionButton.clicked(MouseX,MouseY)
				if len(Trains)>0:
			...
			if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
				...
				menuButton.clicked(MouseX,MouseY)
			...
			if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
				insertOrReplaceButton.clicked(MouseX,MouseY)
	...
	if RUNNING==0:
		...
		if get_setting('datawinLoaded') and get_setting('codeFileLoaded'):
			...
			loadAndInsertButton.draw()
			loadAndInsertButton.draw()
			inPositionText.draw()
			quickInsertPositionButton.draw()
		else:
		...
		if levelSavedNotice > 0:
			screen.blit(fontS.render(f"saved level '{cleanStringForWindowsFileName(missionNameBox.text)}'!",True,(round(levelSavedNotice*0x44),round(levelSavedNotice*0xFF),round(levelSavedNotice*0x44))),(575,575))
	...
	if RUNNING == 11:
		...
		if (get_setting('datawinLoaded') and get_setting('codeFileLoaded')):
			...
			screen.blit(font.render(getInsertionInfo(LevelsInFile[0],selectedLevelInFile[0],insertOrReplaceButton.num),True,'#FFFFFF'),(380,Y));Y = Y+42
			for elem in UI_3:
				elem.draw()
